...
export function ProjectPage({ projectId }: Props) {
    ...
    async function handleRemoveDoc(docId: string) {
        ...
        if (doc && user?.id && doc.user_id && doc.user_id !== user.id) {
            setOwnerOnlyAction("이 문서는 생성자만 삭제할 수 있습니다.");
            return;
        }
    ...
    async function handleTitleCommit(newName: string) {
        ...
        if (project && project.is_owner === false) {
            setOwnerOnlyAction("이 프로젝트는 소유자만 이름을 변경할 수 있습니다.");
            return;
        }
    ...
    async function submitChatRename(chatId: string) {
        ...
        if (chat && user?.id && chat.user_id !== user.id) {
            setOwnerOnlyAction("이 대화는 생성자만 이름을 변경할 수 있습니다.");
            return;
        }
    ...
    async function handleDeleteSelectedDocs() {
        ...
        if (blocked > 0) {
            setOwnerOnlyAction(
                `선택한 문서 ${blocked}개는 삭제할 수 없습니다. 문서는 생성자만 삭제할 수 있습니다.`,
            );
        }
    }
    ...
    async function handleDeleteSelectedChats() {
        ...
        if (blocked > 0) {
            setOwnerOnlyAction(
                `선택한 대화 ${blocked}개는 삭제할 수 없습니다. 대화는 생성자만 삭제할 수 있습니다.`,
            );
        }
    }
    ...
    return (
        <div className="flex-1 overflow-y-auto bg-white flex flex-col h-full">
                ...
                <div>
                    <div className="flex items-center gap-1.5 text-2xl font-medium font-serif">
                        ...
                        {tab !== "documents" && (
                            <>
                                <span className="text-gray-300">›</span>
                                <span className="text-gray-900">{tab === "assistant" ? "어시스턴트" : "표 리뷰"}</span>
                            </>
                        )}
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <HeaderSearchBtn value={search} onChange={setSearch} placeholder="검색…" />
                    <button
                ...
                {tab === "documents" && (
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium select-none shrink-0">
                            ...
                            <div className="ml-auto w-20 shrink-0 text-left">유형</div>
                            <div className="w-24 shrink-0 text-left">크기</div>
                            <div className="w-20 shrink-0 text-left">버전</div>
                            <div className="w-32 shrink-0 text-left">생성일</div>
                            <div className="w-32 shrink-0 text-left">수정일</div>
                            <div className="w-8 shrink-0" />
                        </div>
                        ...
                        {docs.length === 0 && folders.length === 0 ? (
                            ...
                            >
                                ...
                                <p className="text-sm text-gray-400">PDF 또는 DOCX 파일을 여기에 놓으세요</p>
                            </div>
                        ) : (
                            <div
                        ...
                        {contextMenu && (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {contextMenu.showFolderActions ? "하위 폴더 안에 새 폴더" : "새 하위 폴더"}
                                </button>
                ...
                {tab === "assistant" && (
                    <>
                        <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium select-none">
                            ...
                            <div className="ml-auto w-32 shrink-0 text-left">생성일</div>
                            <div className="w-8 shrink-0" />
                        </div>
                        {chats.length === 0 ? (
                            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                <MessageSquare className="h-8 w-8 text-gray-300 mb-4" />
                                <p className="text-2xl font-medium font-serif text-gray-900">어시스턴트</p>
                                <p className="mt-1 text-xs text-gray-400 max-w-xs">이 프로젝트의 문서를 바탕으로 질문하고 답변을 받아보세요.</p>
                                    ...
                                    >
                                        ...
                                        <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${selectedChatIds.includes(chat.id) ? "bg-gray-50" : "bg-white"} group-hover:bg-gray-50`}>
                                            {renamingChatId === chat.id ? (
                                                ...
                                                <span className="text-sm text-gray-800 truncate block">{chat.title ?? "제목 없음"}</span>
                                            )}
                                        </div>
                                        ...
                                        <div className="w-8 shrink-0 flex justify-end" onClick={(e) => e.stopPropagation()}>
                                            <RowActions
                                                onRename={() => {
                                                    ...
                                                    setRenameChatValue(chat.title ?? "제목 없음");
                                                    setRenamingChatId(chat.id);
                                                }}
                                                onDelete={async () => {
                                                    if (user?.id && chat.user_id !== user.id) {
                                                        setOwnerOnlyAction("이 대화는 생성자만 삭제할 수 있습니다.");
                                                        return;
                                                    }
                ...
                {tab === "reviews" && (
                    <>
                        <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium select-none">
                            ...
                            <div className="ml-auto w-24 shrink-0 text-left">열</div>
                            <div className="w-24 shrink-0 text-left">문서</div>
                            <div className="w-32 shrink-0 text-left">생성일</div>
                            <div className="w-8 shrink-0" />
                        </div>
                        {projectReviews.length === 0 ? (
                            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                ...
                                <p className="text-2xl font-medium font-serif text-gray-900">표 리뷰</p>
                                <p className="mt-1 text-xs text-gray-400 max-w-xs">AI로 프로젝트 문서에서 데이터를 표로 추출하세요.</p>
                                    ...
                                    >
                                        ...
                                        <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${selectedReviewIds.includes(review.id) ? "bg-gray-50" : "bg-white"} group-hover:bg-gray-50`}>
                                            {renamingReviewId === review.id ? (
                                                ...
                                                <span className="text-sm text-gray-800 truncate block">{review.title ?? "제목 없음"}</span>
                                            )}
                                        </div>
                                        ...
                                        <div className="w-8 shrink-0 flex justify-end" onClick={(e) => e.stopPropagation()}>
                                            <RowActions
                                                onRename={() => {
                                                    ...
                                                    setRenameReviewValue(review.title ?? "제목 없음");
                                                    setRenamingReviewId(review.id);
                                                }}
                                                onDelete={async () => {
                                                    if (user?.id && review.user_id !== user.id) {
                                                        setOwnerOnlyAction("이 표 리뷰는 생성자만 삭제할 수 있습니다.");
                                                        return;
                                                    }
            ...
            <AddDocumentsModal
                ...
                breadcrumb={["프로젝트", project.name + (project.cm_number ? ` (${project.cm_number})` : ""), "문서 추가"]}
                projectId={projectId}
            />
