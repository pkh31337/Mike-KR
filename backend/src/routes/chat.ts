...
chatRouter.post("/:chatId/generate-title", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const titleText = await completeText({
            model: title_model,
            user: `다음 메시지로 시작하는 법률 AI 플랫폼 채팅의 간결한 제목을 3-6단어로 생성하세요. 제목은 문서 주제나 법률 쟁점을 명확히 표현해야 합니다. '법률 AI', '채팅', '어시스턴트' 등의 단어는 포함하지 마세요. 제목만 반환하고 따옴표나 구두점은 사용하지 마세요. 메시지가 영문이면 영문 제목을 생성하세요.\n\n메시지: ${message.slice(0, 500)}`,
            maxTokens: 64,
            apiKeys: api_keys,
        });
...
chatRouter.post("/", requireAuth, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            let userMsg = "오류가 발생했습니다. 잠시 후 다시 시도해주세요.";
            if (e?.status === 429) {
                userMsg =
                    "AI API 사용 한도를 초과했습니다. Gemini 무료 키는 하루 20회 제한이 있습니다. " +
                    "유료 API 키를 설정하거나 내일 다시 시도해주세요. " +
                    "(Settings → API Keys에서 Claude 또는 Gemini 유료 키를 입력할 수 있습니다.)";
            } else if (e?.status === 401 || e?.status === 403) {
                userMsg =
                    "API 키가 유효하지 않습니다. Settings → API Keys에서 API 키를 확인해주세요.";
            }
            write(
