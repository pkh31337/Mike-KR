...
function formatPromptSuffix(format?: string, tags?: string[]): string {
    switch (format) {
        ...
        case "monetary_amount":
            return ' The "summary" field in your JSON response must be the monetary value only, including currency symbol or currency code (e.g. $1,234.56 or KRW 10,000,000). No explanation.';
        case "currency":
            return ' The "summary" field in your JSON response must contain only the currency code(s). Wrap each code in double square brackets, e.g. [[USD]] or [[KRW]]. No other text.';
        case "yes_no":
            return ' The "summary" field in your JSON response must be [[Yes]] or [[No]] only. For Korean documents, convert "있음"/"없음", "적용"/"미적용", "해당"/"미해당" to [[Yes]] or [[No]] accordingly. The "reasoning" field MUST include an inline citation [[page:N||quote:verbatim excerpt ≤25 words]] pointing to the exact language in the document that supports the Yes/No answer.';
        case "date":
            return ' The "summary" field in your JSON response must be the date only. For Korean documents, output in "YYYY. M. D." format (e.g. 2024. 1. 15.); for non-Korean documents use DD Month YYYY (e.g. 1 January 2024). If a range, give both dates separated by an em dash. The "reasoning" field MUST include an inline citation [[page:N||quote:verbatim excerpt ≤25 words]] pointing to the exact place in the document where the date is found.';
        case "tag":
            return tags?.length
...
function buildTabularMessages(
    ...
    const systemContent = `You are 미케, an AI legal assistant. You are helping with the tabular review titled "${reviewTitle}".
