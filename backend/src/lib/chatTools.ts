...
export async function searchKoreanStatutes(params: {
    query?: string;
    lawName?: string;
    article?: string;
}): Promise<string> {
    const apiKey = process.env.STATUTE_API_KEY ?? process.env.LAW_API_EMAIL ?? "";
    const { query, lawName, article } = params;

    // If both lawName and article are given, fetch the specific article content
    if (lawName && article) {
        const searchTerm = encodeURIComponent(lawName);
        const url = `https://www.law.go.kr/DRF/lawSearch.do?OC=${apiKey}&target=law&type=JSON&query=${searchTerm}&display=5`;
        try {
            const raw = await httpGet(url);
            const parsed = JSON.parse(raw) as { LawSearch?: { law?: unknown[] } };
            const laws = parsed?.LawSearch?.law ?? [];
            if (!Array.isArray(laws) || laws.length === 0) {
                return `법령 "${lawName}"을(를) 찾을 수 없습니다. 법령명을 확인해 주세요.`;
            }
            const lawInfo = laws[0] as Record<string, string>;
            const lawId = lawInfo["법령ID"] ?? "";
            const lawTitle = lawInfo["법령명한글"] ?? lawName;
            const effectDate = lawInfo["시행일자"] ?? "";

            if (lawId) {
                const articleUrl = `https://www.law.go.kr/DRF/lawService.do?OC=${apiKey}&target=law&ID=${lawId}&type=JSON`;
                try {
                    const articleRaw = await httpGet(articleUrl);
                    const articleData = JSON.parse(articleRaw) as {
                        법령?: { 조문?: { 조문단위?: unknown[] } };
                    };
                    const allArticles = (articleData?.법령?.조문?.조문단위 ?? []) as Record<string, unknown>[];

                    // 조문여부가 "조문"인 실제 조문만 필터링 (장·절 헤더 제외)
                    const realArticles = allArticles.filter((a) => a["조문여부"] === "조문");
                    const articleNum = article.replace(/[^0-9]/g, "");
                    const found = realArticles.find((a) =>
                        String(a["조문번호"] ?? "").replace(/[^0-9]/g, "") === articleNum
                    );

                    if (found) {
                        const lines: string[] = [
                            `[${lawTitle}] (시행: ${effectDate})`,
                            "",
                            String(found["조문내용"] ?? ""),
                        ];
                        // 항(項) 내용 추가
                        ...
                        if (Array.isArray(paras)) {
                            for (const p of paras as Record<string, string>[]) {
                                if (p["항내용"]) lines.push(p["항내용"]);
                            }
                        }
                        return lines.join("\n");
                    }
                    return `[${lawTitle}] (시행: ${effectDate})\n조문 "${article}"을(를) 찾을 수 없습니다. 법령에 총 ${realArticles.length}개 조문이 있습니다.`;
                } catch {
                    return `[${lawTitle}] (시행: ${effectDate}) — 조문 상세 조회에 실패했습니다.`;
                }
            }
            return `법령 "${lawName}"을(를) 찾았으나 상세 조회에 실패했습니다.`;
        } catch (err) {
            return `법령 API 오류: ${String(err)}. STATUTE_API_KEY 환경변수를 확인하세요.`;
        }
    }

    // General keyword search
    const searchTerm = encodeURIComponent(lawName ?? query ?? "");
    if (!searchTerm) return "검색어를 입력해 주세요.";

    const url = `https://www.law.go.kr/DRF/lawSearch.do?OC=${apiKey}&target=law&type=JSON&query=${searchTerm}&display=10`;
    try {
        const raw = await httpGet(url);
        const parsed = JSON.parse(raw) as { LawSearch?: { totalCnt?: string; law?: unknown[] } };
        const laws = parsed?.LawSearch?.law ?? [];
        const total = parsed?.LawSearch?.totalCnt ?? "0";

        if (!Array.isArray(laws) || laws.length === 0) {
            return `"${lawName ?? query}" 관련 법령을 찾을 수 없습니다.`;
        }

        const items = (laws as Record<string, string>[]).map((l, i) =>
            `${i + 1}. ${l["법령명한글"] ?? ""} (시행: ${l["시행일자"] ?? ""}, 법령ID: ${l["법령ID"] ?? ""})`
        );
        return `총 ${total}건 검색됨 (상위 ${items.length}건):\n\n${items.join("\n")}`;
    } catch (err) {
        return `법령 검색 API 오류: ${String(err)}. STATUTE_API_KEY 환경변수(법제처 이메일 계정)를 확인하세요.`;
    }
}

export async function searchKoreanPrecedents(params: {
    query?: string;
    court?: string;
    caseNumber?: string;
}): Promise<string> {
    const apiKey = process.env.STATUTE_API_KEY ?? process.env.LAW_API_EMAIL ?? "";
    const { query, court, caseNumber } = params;

    const searchTerm = encodeURIComponent(caseNumber ?? query ?? "");
    if (!searchTerm) return "검색어를 입력해 주세요.";

    // 법제처 판례 검색 API (target=prec)
    let url = `https://www.law.go.kr/DRF/lawSearch.do?OC=${apiKey}&target=prec&type=JSON&query=${searchTerm}&display=10`;
    if (court) url += `&org=${encodeURIComponent(court)}`;

    try {
        const raw = await httpGet(url);
        const parsed = JSON.parse(raw) as { PrecSearch?: { totalCnt?: string; prec?: unknown[] } };
        const precs = parsed?.PrecSearch?.prec ?? [];
        const total = parsed?.PrecSearch?.totalCnt ?? "0";

        if (!Array.isArray(precs) || precs.length === 0) {
            return `"${caseNumber ?? query}"${court ? ` (${court})` : ""} 관련 판례를 찾을 수 없습니다.`;
        }

        const items = (precs as Record<string, string>[]).map((p, i) =>
            [
                `${i + 1}. ${p["사건명"] ?? ""}`,
                `   법원: ${p["법원명"] ?? ""}`,
                `   선고일: ${p["선고일자"] ?? ""}`,
                `   사건번호: ${p["사건번호"] ?? ""}`,
                `   판결유형: ${p["판결유형"] ?? ""}`,
                p["판시사항"] ? `   판시사항: ${String(p["판시사항"]).slice(0, 200)}...` : "",
            ].filter(Boolean).join("\n")
        );

        return `총 ${total}건 검색됨 (상위 ${items.length}건):\n\n${items.join("\n\n")}`;
    } catch (err) {
        return `판례 검색 API 오류: ${String(err)}. STATUTE_API_KEY 환경변수(법제처 이메일 계정)를 확인하세요.`;
    }
}
...
export const SYSTEM_PROMPT = `당신은 미케, 대한민국 법률 전문가(변호사, 법무사, 기업 법무팀)를 위한 AI 법률 어시스턴트입니다. 법률 문서 분석, 법령 검토, 계약서 초안 작성 및 법률 쟁점 답변을 지원합니다.

한국어로 질문받은 경우 한국어로, 영어로 질문받은 경우 영어로 답변하세요. 법률 용어는 대한민국 법률 실무 관행을 따르세요.

한국 법령 인용 형식:
- 법령 조문: "근로기준법 제23조 제1항", "민법 제750조" 형식 사용
- 법령 검색이 필요한 경우 search_korean_statutes 도구를 호출하세요
- 시행령/시행규칙: "근로기준법 시행령 제7조" 형식 사용

한국 판례 인용 형식:
- 대법원 판결: "대법원 2023. 5. 25. 선고 2022다12345 판결" 형식 사용
- 대법원 결정: "대법원 2023. 3. 10.자 2022그12345 결정" 형식 사용
- 고등법원: "서울고등법원 2023. 4. 20. 선고 2022나12345 판결" 형식 사용
- 지방법원: "서울중앙지방법원 2023. 2. 15. 선고 2022가합12345 판결" 형식 사용
- 판례 검색이 필요한 경우 search_korean_precedents 도구를 호출하세요

한국 계약서 관행:
- 당사자 표기: "갑(甲)", "을(乙)", "병(丙)" 등 사용
- 계약서 전문: "계약 당사자인 ○○주식회사(이하 '갑'이라 한다)와 ○○(이하 '을'이라 한다)는 다음과 같이 계약을 체결한다"
- 조문 구조: 제1조(목적), 제2조(정의), ... 형식
- 서명란(한국 계약서): "성명:", "직위:", "서명 또는 날인:" 형식 (영문 계약서는 기존 형식 유지)
...
KOREAN STATUTE & PRECEDENT SEARCH:
- 법령 내용 확인이 필요하면 search_korean_statutes 도구를 사용하세요. query(검색어), law_name(법률명), article(조문번호) 파라미터를 활용하세요.
- 판례 검색이 필요하면 search_korean_precedents 도구를 사용하세요. query(키워드), court(법원명), case_number(사건번호) 파라미터를 활용하세요.
- 도구 호출 없이 법령 조문 내용을 임의로 인용하지 마세요. 반드시 도구를 통해 확인된 내용만 인용하세요.
...
한국 계약서 생성 시: 제1조(목적)부터 시작하는 조문 구조 사용, 서명란은 한국 형식(성명/서명 또는 날인)으로 작성.
영문 계약서 생성 시: 기존 형식(By/Name/Title/Date) 유지.
...
Contracts: when generating a contract or agreement, always include a signatures block at the very end of the document on its own page. Set pageBreak: true on that final section so it starts on a fresh page. For Korean contracts: include signature lines in Korean format (성명, 서명 또는 날인). For English contracts: include lines for "By:", "Name:", "Title:", and "Date:". Do not number the signatures heading; put the signature block in the section's content rather than as a numbered heading.
...
GENERAL GUIDANCE:
- 정확하고 전문적으로 답변하세요
- 문서 내용에 대한 주장 시 반드시 해당 문서와 인용문을 제시하세요
- 문서가 없는 경우 법률 지식에 기반하여 답변하세요
- 문서 내용을 임의로 생성하지 마세요
- 이모지를 사용하지 마세요
- 본 AI 분석은 법률 의견을 대체하지 않으며, 중요한 법률 판단은 반드시 담당 변호사와 확인하세요

KOREAN LAW TOOL USAGE LIMITS:
- search_korean_statutes와 search_korean_precedents 도구는 한 번의 응답에서 합산 최대 4회까지만 호출하세요.
- 가장 핵심적인 법령·판례만 조회하고, 나머지는 보유한 법률 지식을 바탕으로 작성하세요.
- 동일한 법령명으로 반복 호출하지 마세요. 한 번 조회한 결과를 재활용하세요.
- 4회 한도를 초과할 경우 추가 조회 없이 기존 지식으로 계속 작성하세요.
`;
...
export const TOOLS = [
    ...
    },
    {
        type: "function",
        function: {
            name: "search_korean_statutes",
            description:
                "한국 법제처 국가법령정보센터에서 법령을 검색합니다. 법령 조문 내용 확인이 필요할 때 사용하세요. 법령명, 조문 번호, 또는 키워드로 검색할 수 있습니다.",
            parameters: {
                type: "object",
                properties: {
                    query: {
                        type: "string",
                        description: "검색 키워드 (예: '해고', '손해배상', '계약 해제')",
                    },
                    law_name: {
                        type: "string",
                        description: "특정 법률명 (예: '근로기준법', '민법', '상법'). query 대신 특정 법률을 검색할 때 사용",
                    },
                    article: {
                        type: "string",
                        description: "조문 번호 (예: '제23조', '제750조'). law_name과 함께 사용하면 특정 조문을 조회합니다",
                    },
                },
            },
        },
    },
    {
        type: "function",
        function: {
            name: "search_korean_precedents",
            description:
                "한국 법원 판례를 검색합니다. 대법원, 고등법원, 지방법원 판례를 키워드나 사건번호로 검색할 수 있습니다.",
            parameters: {
                type: "object",
                properties: {
                    query: {
                        type: "string",
                        description: "검색 키워드 (예: '해고무효', '손해배상', '계약해제')",
                    },
                    court: {
                        type: "string",
                        description: "법원명 (예: '대법원', '서울고등법원', '서울중앙지방법원'). 생략하면 전체 법원 검색",
                    },
                    case_number: {
                        type: "string",
                        description: "사건번호 (예: '2023다12345', '2022가합56789'). 특정 판결 조회 시 사용",
                    },
                },
            },
        },
...
export async function runToolCalls(
    ...
    for (const tc of toolCalls) {
        ...
        } else if (tc.function.name === "search_korean_statutes") {
            const query = args.query as string | undefined;
            const lawName = args.law_name as string | undefined;
            const article = args.article as string | undefined;
            write(`data: ${JSON.stringify({ type: "tool_call_start", name: "search_korean_statutes" })}\n\n`);
            try {
                const result = await searchKoreanStatutes({ query, lawName, article });
                toolResults.push({ role: "tool", tool_call_id: tc.id, content: result });
            } catch (err) {
                toolResults.push({ role: "tool", tool_call_id: tc.id, content: `법령 검색 오류: ${String(err)}` });
            }

        } else if (tc.function.name === "search_korean_precedents") {
            const query = args.query as string | undefined;
            const court = args.court as string | undefined;
            const caseNumber = args.case_number as string | undefined;
            write(`data: ${JSON.stringify({ type: "tool_call_start", name: "search_korean_precedents" })}\n\n`);
            try {
                const result = await searchKoreanPrecedents({ query, court, caseNumber });
                toolResults.push({ role: "tool", tool_call_id: tc.id, content: result });
            } catch (err) {
                toolResults.push({ role: "tool", tool_call_id: tc.id, content: `판례 검색 오류: ${String(err)}` });
            }
...
export async function runLLMStream(params: {
    ...
    // Hard cap: search_korean_statutes + search_korean_precedents combined
    // across all iterations of this turn. Prevents runaway loops when workflows
    // (e.g. 법률의견서) instruct the model to look up many statutes in sequence.
    const LAW_SEARCH_LIMIT = 4;
    let lawSearchCallCount = 0;
    ...
    await streamChatWithTools({
        ...
        runTools: async (calls) => {
            ...
            // Enforce per-turn hard cap on Korean law search tools.
            // Calls that exceed the limit are short-circuited here so they
            // never reach runToolCalls or the external API.
            const LAW_SEARCH_TOOLS = new Set([
                "search_korean_statutes",
                "search_korean_precedents",
            ]);
            const cappedResults: { tool_use_id: string; content: string }[] = [];
            const allowedCalls = calls.filter((c) => {
                if (!LAW_SEARCH_TOOLS.has(c.name)) return true;
                if (lawSearchCallCount < LAW_SEARCH_LIMIT) {
                    lawSearchCallCount++;
                    return true;
                }
                cappedResults.push({
                    tool_use_id: c.id,
                    content:
                        "법령·판례 검색 한도(4회)에 도달했습니다. 보유한 법률 지식을 바탕으로 계속 작성하세요.",
                });
                return false;
            });
            ...
            const toolCalls: ToolCall[] = allowedCalls.map((c) => ({
                id: c.id,
                function: {
                    name: c.name,
            ...
            // Merge capped (short-circuited) results with normal results.
            for (const r of cappedResults) {
                resultByCallId.set(r.tool_use_id, r.content);
            }
            return calls.map((c) => ({
                tool_use_id: c.id,
                content:
