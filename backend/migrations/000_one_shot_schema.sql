...
-- No additional policies are needed for backend-only tables.
-- ---------------------------------------------------------------------------
-- [보충설명]
-- Enable RLS on all public tables that were missing it.
-- The backend uses the service-role key which bypasses RLS, so no
-- additional policies are needed for backend-only tables.
-- user_profiles already has RLS + per-user SELECT/UPDATE policies.
